...
overview_c <- dataset %>% mutate(r = as.factor(Result)) %>% 
  select(File, Parser, Category, r) %>% 
  mutate(Library = gsub("yasjf4j-", "", Parser)) %>%
  group_by(Library, Category, r, .drop=FALSE) %>% 
  summarize(n = n()) %>% 
  ...
  mutate(totExpected = OK + EQUIVALENT_OBJECT, totSilent = NON_EQUIVALENT_OBJECT, totError = NULL_OBJECT + PARSE_EXCEPTION + PRINT_EXCEPTION + CRASH) %>%
  mutate(rex = (totExpected / totPerLine), rs = (totSilent / totPerLine), rer = (totError / totPerLine)) %>%
  select(Library,Category,rex,rs,rer)

overview_c[ overview_c == "jackson-databind" ] <- "jackson"
...
overview_i <- dataset %>% mutate(r = as.factor(Result)) %>% 
  select(File, Parser, Category, r) %>% 
  mutate(Library = gsub("yasjf4j-", "", Parser)) %>%
  group_by(Library, Category, r, .drop=FALSE) %>% 
  summarize(n = n()) %>% 
...
overview_i[ overview_i == "jackson-databind" ] <- "jackson"

over <- rbind(overview_c,overview_i) %>% mutate(Expected=rex,Silent=rs,Error=rer) %>% mutate(Corpus=Category) %>% select(Library,Corpus,Expected,Silent,Error)
over <- over %>% group_by(Library,Corpus) %>% gather(key = "Outcome", value = "Share", -Corpus, -Library) %>% mutate(Share = 100 * Share)
over[ over == "correct" ] <- "Well-formed"
over[ over == "ill-formed" ] <- "Ill-formed"

over$Outcome <- factor(over$Outcome , levels=c("Expected", "Silent", "Error"))
over$Corpus <- factor(over$Corpus , levels=c("Well-formed", "Ill-formed"))

over %>%
  ggplot(aes(x = Outcome, y = Share)) + 
  geom_boxplot(aes(fill = Corpus)) +
  scale_fill_manual(values=c("#22AA22", "#AA2222")) +
  theme_bw() + theme(text = element_text(size=20))
ggsave("overview_json_outcome.svg")
```
